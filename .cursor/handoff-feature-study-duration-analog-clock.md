...
`origin/develop` を取り込み済み。`0007` は本番/develop の `record_reactions` を維持し、学習時間マイグレーションは `0008` / `0009` に繰り下げた。
...
- 刻みは 5分（5〜720）。`0008` の 10分 CHECK を `migrations/0009_duration_minutes_five_minute_step.sql` で更新
...
`DurationMinutesPicker.tsx` / `StudyDatetimePicker.tsx` / `recordFormUtils.ts` / `shared/schemas.ts` / `migrations/0008_*.sql` / `migrations/0009_*.sql`
