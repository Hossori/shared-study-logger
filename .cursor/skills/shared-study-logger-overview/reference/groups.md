...
- **関連ファイル**:
  - Worker: `src/worker/routes/groups.ts`（`GET /api/groups` 所属グループのみ、
    `GET /:groupId/members` 所属メンバー公開情報）、
    `src/worker/routes/admin-directory.ts`（`/api/admin/groups` 全件・作成・所属変更）、
    `src/worker/lib/db.ts`の`getGroupsForUser`/`isUserInGroup`/`listGroupMembers`/
    `listAllGroupsWithMembers`/`createGroup`/`addGroupMember`/`removeGroupMember`
  - フロント: `src/react-app/features/groups/GroupSwitcher.tsx`、
    `src/react-app/features/groups/AdminDirectoryPage.tsx`（管理者のグループユーザー管理）、
    `src/react-app/queries/useGroups.ts`（`useGroupsQuery`、`useGroupMembersQuery`）、
...
- **データフロー**: `GET /api/groups`（`requireAuth`必須、`index.ts`で
  ...
  自分の所属グループ一覧を取得。`GET /api/groups/:groupId/members` は所属チェック後
  `listGroupMembers` で `id` / `displayName` / `avatarKey` のみ返す（非所属・存在しないグループは 403）。
  フロントは取得後、`GroupSwitcher`が未選択または選択中グループが
