...
- インデックス: `group_members(user_id)`、`study_records(group_id, study_datetime DESC, id DESC)`（`idx_study_records_group_sort`。カーソルページネーション用）、`study_records(user_id)`、`record_reactions(record_id)`、`push_subscriptions(user_id)`、`app_notifications(enabled, created_at DESC)`。
