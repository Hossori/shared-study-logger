...
function buildStudyRecordStatements(groupId, count, resolveUserId) {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    statements.push(
      `INSERT OR IGNORE INTO study_records (id, group_id, user_id, study_datetime, title, duration_minutes, memo, created_at, updated_at) VALUES ('${seedRecordId(i)}', '${groupId}', '${userId}', '${studyDatetime}', '${sqlEscape(title)}', 60, ${memoSql}, '${createdAt}', '${createdAt}');`,
    );
  }
  ...
  return buildSeedSql().then(({ sql, users, groupIds }) => {
    ...
    groupIds.forEach((id, i) => {
      ...
      if (group.recordCount === 0) {
        console.log(`# group[${i}].id = ${id} (${group.name}, records=0)`);
        return;
      }
